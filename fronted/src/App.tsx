...
function AccountModal({ form, setForm, accounts, editing, close, save }: { form: any; setForm: any; accounts: Account[]; editing: Account | null; close: () => void; save: (e: FormEvent) => void }) {
  ...
  return <div className="overlay"><form className="modal" onSubmit={save}><div className="modal-head"><div><p className="eyebrow">CHART OF ACCOUNTS</p><h2>{editing ? 'Edit account' : 'New account'}</h2></div><button type="button" className="close" onClick={close}>×</button></div><div className="form-grid"><label>Account name<input required value={form.name} onChange={e => handleNameChange(e.target.value)} placeholder="e.g. Operating Cash" /></label><label>Account code (exactly 5 digits)<input required value={form.code} onChange={e => field('code', e.target.value.replace(/\D/g, '').slice(0, 5))} placeholder="e.g. 11101" className={!isCodeValid && form.code ? 'border-red-500' : ''} /></label><label>Account type<select value={form.type} onChange={e => handleTypeChange(e.target.value)}>{accountTypes.map(x => <option key={x}>{x}</option>)}</select></label><label>Account Subtype<select value={subtype} onChange={e => handleSubtypeChange(e.target.value)}>{activeSubtypes.map(x => <option key={x} value={x}>{x}</option>)}</select></label><label>Parent account (Suggested)<select value={form.parentId} onChange={e => { field('parentId', e.target.value); if (!editing) fetchNextCode(form.type, e.target.value); }}><option value="">No parent (top-level)</option>{filteredParents.map(a => <option value={a.id} key={a.id}>{a.code} — {a.name}</option>)}</select></label><label>Opening balance<input type="number" step="0.01" value={form.openingBalance} onChange={e => field('openingBalance', e.target.value)} /></label><label>IFRS tag<input value={form.ifrsTag} onChange={e => field('ifrsTag', e.target.value)} placeholder="Optional IFRS tag" /></label><label>GAAP tag<input value={form.gaapTag} onChange={e => field('gaapTag', e.target.value)} placeholder="Optional GAAP tag" /></label></div><label className="check"><input type="checkbox" checked={form.reconciliationEnabled} onChange={e => field('reconciliationEnabled', e.target.checked)} /> Enable account reconciliation</label><div className="modal-footer"><button type="button" className="secondary" onClick={close}>Cancel</button><button className="primary" disabled={!isCodeValid || !form.name.trim()}>{editing ? 'Save changes' : 'Create account'}</button></div></form></div>
}
