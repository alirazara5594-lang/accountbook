...
<button className="primary">Create allocation</button></div></form></section><section className="panel allocation-list"><div className="panel-head"><div><h3>Active allocations</h3><p>Rate × quantity for each billing period.</p></div><DataToolbar exportFileName="intercompany-allocations" exportSheetName="Intercompany Allocations" exportTitle="Intercompany Allocations" exportSubtitle="Shared-cost allocations across group companies." exportHeaders={exportHeaders} exportRows={exportRows} onRefresh={reload} /></div>{allocations.length ? allocations.map(a => <div className="allocation" key={a.id}><div><strong>{a.name}</strong><small>{companyName(a.sourceCompanyId)} · {a.category} · {a.frequency}</small></div><div><b>{money(a.rate * a.quantity)}</b><small>{a.status} · {a.recipients.map(x => `${companyName(x.companyId)} ${x.sharePercent}%`).join(' · ')}</small></div><div className="allocation-actions"><button onClick={() => processAllocation(a.id)} className="btn-sm btn-primary" disabled={a.status !== 'Active'}>Process</button>{a.status === 'Active' ? <button onClick={() => updateStatus(a.id, 'Paused')} className="btn-sm btn-secondary">Pause</button> : <button onClick={() => updateStatus(a.id, 'Active')} className="btn-sm btn-secondary">Activate</button>}</div></div>) : <div className="empty">No allocations yet.</div>}</section></div>
}
